...
use zbus_address::transport::TcpFamily;
...
pub(crate) async fn connect(addr: &zbus_address::transport::Tcp<'_>) -> Result<Stream> {
...
pub(crate) async fn connect_nonce(addr: &zbus_address::transport::NonceTcp<'_>) -> Result<Stream> {
    let Some(host) = addr.host() else {
        return Err(Error::Address("No host in address".into()));
    };
    let Some(port) = addr.port() else {
    ...
    fn connect_nonce_tcp() {
        ...
        impl std::fmt::Display for PercentEncoded<'_> {
            fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                zbus_address::encode_percents(f, self.0)
            }
        }
